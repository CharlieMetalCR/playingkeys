...
- React Native + Expo (Expo Go for development, EAS for production).
- TypeScript strict.
- Expo Router for file-based routing.
- NativeWind for Tailwind-style styling.
- TanStack Query for remote data.
- Lucide icons for all UI elements.
- expo-av for piano sound playback.
...
- PostgreSQL (database name: plyngpDB).
- Prisma for ORM and migrations.
- Zod and validation pipes at request boundaries.
...
- Use Lucide icons exclusively. Do not use emoji as product UI icons.
...
- Piano keys must produce accurate sound when tapped.
- Lesson content must be clear and pedagogically sound.
